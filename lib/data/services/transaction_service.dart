...
class TransactionService extends GetConnect {
  ...
  Future<Response> createNewTransaction(Transaction newTrans) async {
    return await post("$baseUrl/wallets/${newTrans.walletId}/transactions", jsonEncode(newTrans),
  ...
  Future<Response> updateTransaction(Transaction upTrans) async {
    return await put(
        "$baseUrl/wallets/${upTrans.walletId}/transactions/${upTrans.id}", jsonEncode(upTrans),
        headers: <String, String>{
          'Authorization': _ac.currentUser.value!.token!,
        });
  }
  ...
  Future<Response> getTransactionByWalletId(
    ...
    return await get("$baseUrl/wallets/$walletId/transactions", query: <String, String>{
  ...
  Future<Response> getGlobalTransaction(
    ...
    return await get("$baseUrl/global-wallets/transactions", query: <String, String>{
      "StartDate": _getStartDate(timeRange),
      "EndDate": _getEndDate(timeRange),
    }, headers: <String, String>{
      'Authorization': _ac.currentUser.value!.token!,
  ...
  Future<Response> deleteTransaction(Transaction delTrans) async {
    return await delete("$baseUrl/wallets/${delTrans.walletId}/transactions/${delTrans.id}",
        headers: <String, String>{
          'Authorization': _ac.currentUser.value!.token!,
        });
  }
  ...
  Future<Response> getTransactionByTransactionId(int walletId, int transactionId) async {
    return await get("$baseUrl/wallets/$walletId/transactions/$transactionId",
        headers: <String, String>{
          'Authorization': _ac.currentUser.value!.token!,
        });
  }
  ...
  Future<Response> getTransactionOfEvent(int walletId, int eventId) async {
    return await get("$baseUrl/wallets/$walletId/transactions?EventId=$eventId",
        headers: <String, String>{
          'Authorization': _ac.currentUser.value!.token!,
        });
  }
  ...
  Future<Response> getRecentlyTrans() async {
    return await get("$baseUrl/global-wallets/transactions/recently?Take=5",
        headers: <String, String>{
          'Authorization': _ac.currentUser.value!.token!,
        });
  }
  ...
  _getEndDate(String timeRange) {
    ...
    var dayOfThisTime = DateUtils.getDaysInMonth(int.parse(ele[1]), int.parse(ele[0]));
