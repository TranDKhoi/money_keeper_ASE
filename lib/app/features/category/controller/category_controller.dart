...
import '../widgets/expanded_list_tile.dart';
...
class CategoryController extends GetxController {
  ...
  void changeListCategory(int index) {
    ...
    switch (index) {
      case 0:
        listIncome.value = List.from(listCate.where((p0) => p0.type == "Income"));
        break;
      case 1:
        {
          listExpense.value = List.from(listCate.where((p0) => p0.type == "Expense"));

          listExpenseTile.value = <BasicTile>[
            BasicTile(
              tileName: R.RequiredExpense.tr,
              tiles: listExpense.where((p0) => p0.group == "RequiredExpense").toList(),
            ),
            BasicTile(
              tileName: R.NecessaryExpense.tr,
              tiles: listExpense.where((p0) => p0.group == "NecessaryExpense").toList(),
            ),
            BasicTile(
              tileName: R.Entertainment.tr,
              tiles: listExpense.where((p0) => p0.group == "Entertainment").toList(),
            ),
            BasicTile(
              tileName: R.InvestingOrDebt.tr,
              tiles: listExpense.where((p0) => p0.group == "InvestingOrDebt").toList(),
            ),
          ];
          break;
        }
