  ...
  getSummaryData() async {
    if (selectedReport.value == 0) {
      await Future.wait([
        ...
        GlobalWalletService.ins
            .getExpenseReportByWeek(
                DateTime.now().subtract(Duration(days: DateTime.now().weekday - 1)).day - 7)
            .then((value) {
          if (value.isOk) {
      ...
      await Future.wait([
        GlobalWalletService.ins.getExpenseReportByMonth(DateTime.now()).then((value) {
          if (value.isOk) {
            barChartData[1] = value.data["totalAmount"];
          } else {
            EasyLoading.showToast(value.errorMessage);
        ...
        GlobalWalletService.ins
            .getExpenseReportByMonth(DateTime(DateTime.now().year, DateTime.now().month - 1))
            .then((value) {
          if (value.isOk) {
            barChartData[0] = value.data["totalAmount"];
          } else {
