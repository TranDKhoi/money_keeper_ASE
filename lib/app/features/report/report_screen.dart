...
import 'package:money_keeper/app/features/report/widgets/report_line_chart.dart';
import 'package:money_keeper/app/features/report/widgets/report_pie_chart.dart';
...
class _ReportScreenState extends State<ReportScreen> with SingleTickerProviderStateMixin {
  ...
  @override
  void initState() {
    _tabController = TabController(length: _controller.listTimeline.length, vsync: this);
      ...
      appBar: AppBar(
        ...
        title: Column(
          children: [
            Text(R.Balance.tr),
            Obx(() => Text(
                FormatHelper().moneyFormat(_controller.selectedWallet.value.balance?.toDouble()))),
          ],
        ),
            ...
            children: [
              ...
              Obx(
                () => Text(
                  ...
                  style: TextStyle(
                    fontSize: 25,
                    color: _controller.summary.value < 0 ? Colors.red : Colors.green,
                  ),
                ),
              ),
              const SizedBox(height: 10),
              ...
              Obx(() {
                if (_controller.incomeSummary.value != 0) {
                  return Row(
                    children: [
                      Expanded(
                          child: ReportPieChart(pieData: _controller.incomePie, type: "Income")),
                    ],
                  );
                }
                return const SizedBox.shrink();
                  ...
                  if (_controller.expenseSummary.value != 0) {
                    return Row(
                      children: [
                        Expanded(
                            child:
                                ReportPieChart(pieData: _controller.expensePie, type: "Expense")),
                      ],
                    );
                  } else {
                    return const SizedBox.shrink();
