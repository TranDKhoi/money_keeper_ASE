...
class ReportController extends GetxController {
  ...
  initData() async {
    ...
    Wallet globalWallet = Wallet(name: R.Totalwallet.tr, id: -1, balance: _calculateTotalBalance());
  ...
  Future<void> _getLineData() async {
    ...
    if (selectedWallet.value.id == -1) {
      res = await ReportService.ins.getDailyReportGlobal(timeRange: selectedTimeLine.value!);
    } else {
    ...
    if (res.isOk) {
      ...
      final int maxDy = (maxIncome > maxExpense ? maxIncome : maxExpense).ceil();
  ...
  void _getIncomeData() {
    if (selectedWallet.value.id == -1) {
      ReportService.ins.getIncomeReportGlobal(timeRange: selectedTimeLine.value!).then((res) {
        if (res.isOk) {
          incomePie.value = [];
  ...
  void _getExpenseData() {
    if (selectedWallet.value.id == -1) {
      ReportService.ins.getExpenseReportGlobal(timeRange: selectedTimeLine.value!).then((res) {
        if (res.isOk) {
          expensePie.value = [];
