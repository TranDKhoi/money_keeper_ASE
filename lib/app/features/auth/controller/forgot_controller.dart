...
class ForgotPassController extends GetxController {
  ...
  void toVerifyScreen() async {
    if (emailTextController.text.isNotEmpty) {
      EasyLoading.show();
      var res = await AuthService.ins.forgotPassword(email: emailTextController.text);
      EasyLoading.dismiss();
  ...
  void verifyCodeFunc() async {
    if (secureCode != null) {
      ...
      var res = await AuthService.ins
          .verifyResetPassword(email: emailTextController.text, code: secureCode!);
      EasyLoading.dismiss();
  ...
  void setNewPassFunc() async {
    if (passTextController.text.isNotEmpty && rePassTextController.text.isNotEmpty) {
      if (passTextController.text == rePassTextController.text) {
        EasyLoading.show();
        var res = await AuthService.ins
            .resetPassword(token: resetPassToken!, password: rePassTextController.text);
        EasyLoading.dismiss();
