...
class BudgetController extends GetxController {
  ...
  void calculatePositionByMonth() {
    ...
    int dayInMonth = DTU.getDaysInMonth(DateTime.now().year, DateTime.now().month);
    if (today == 1) today = 0;
  ...
  Future<void> createBudget() async {
    ...
    try {
      CreateBudget createBudget = CreateBudget(
        limitAmount: int.parse(limitAmount.text.trim().replaceAll(RegExp(r"\D"), "")),
  ...
  Future<void> initBudgetInfoScreenData({required int budgetId}) async {
    ...
    for (int i = DateTime.now().day;
      ...
      if (statistics[i].expenseAmount != null) {
        ...
        statistics[i].expenseAmount = temp + budgetDetailSummary.value!.expectedExpense!.round();
  ...
  Future<void> getBudgetDetailSummary() async {
    try {
      ...
      if (res.isOk) {
        budgetDetailSummary.value = BudgetDetailSummary.fromJson(res.body["data"]);
      }
  ...
  Future<void> getBudgetStatistic() async {
    try {
      ...
      if (res.isOk) {
        statistics =
            List.from(res.body["data"].map((json) => BudgetDetailStatistic.fromJson(json)));
      }
      log(res.body["message"]);
    } catch (e) {
  ...
  Future<void> getBudgetsInMonth() async {
    try {
      ...
      if (res.isOk) {
        budgets.value = List.from(res.data.map((budget) => Budget.fromJson(budget)));
      }
      log(res.body["message"]);
    } catch (e) {
